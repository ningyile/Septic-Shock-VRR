...
gp_select <- c("50% ≤ VRR", "0 ≤ VRR < 50%", "-50% ≤ VRR < 0", "VRR < -50%")[c(2,4)]
col_select <- c("#75D054FF", "#2C728EFF", "#FB5D0F", "#FB75D3")[c(2,4)]

f='./load/matched_cohort5.Rdata'
...
if (!file.exists(f)) {
  ...
  tbl_summary_matching = merge(tbl_before_matcing, tbl_after_matcing, by = 'Var1', all = T) %>% 
    ...
    flextable() %>% 
    set_caption(., "The number of SMD greater than 0.1 before and after matching for cohort5") 
  tbl_summary_matching
  tbl_smd5 = tbl_summary_matching
  save(tbl_smd5, file ="./load/tbl/tbl_smd5.Rdata" )
...
tbl
tbl40 <- tbl
save(tbl40, file ="./load/tbl/tbl40.Rdata")

# 匹配后table1
tbl = make_table1(fml,matched_df)
tbl
tbl41 <- tbl
save(tbl41, file ="./load/tbl/tbl41.Rdata")
...
tbl42 <- tbl_regression(res.svyglm,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted generalised linear model of with all covariates using IPW for ICU mortality of cohort5") 
tbl42
save(tbl42, file ="./load/tbl/tbl42.Rdata" )
...
tbl43 <- tbl_regression(res.svycox,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted Cox model with all covariates using IPW for ICU mortality of cohort5") 
tbl43
save(tbl43, file ="./load/tbl/tbl43.Rdata" )
...
## km曲线
p = ggsurvplot(fit,
               title = "Kaplan-Meier survival curve for ICU mortality of matched cohort5",
               xlab = "Time(Days)", 
...
full_plot
p24=full_plot
save(p24,file = './load/img/p24.Rdata')
...
tbl44 <- tbl_regression(res.cox, exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Multivariate Cox regression analysis for ICU mortality of matched cohort5") 
tbl44
save(tbl44, file ="./load/tbl/tbl44.Rdata" )

## 森林图
p = ggforest(
  res.cox,
  data = matched_df,
  main = "Forest plot of multivariate Cox regression analysis for ICU mortality of matched cohort5
...
p
p25=p
save(p25,file = './load/img/p25.Rdata')
...
tbl45 <- tbl_regression(res.svyglm,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted generalised linear model of with all covariates using IPW for in hospital mortality of cohort5") 
tbl45
save(tbl45, file ="./load/tbl/tbl45.Rdata" )
...
tbl46 <- tbl_regression(res.svycox,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted Cox model with all covariates using IPW for in hospital mortality of cohort5") 
tbl46
save(tbl46, file ="./load/tbl/tbl46.Rdata" )
...
## km曲线
p = ggsurvplot(fit,
               title = "Kaplan-Meier survival curve for in hospital mortality of matched cohort5",
               xlab = "Time(Days)", 
               ylab = "Surivival probability",
               xlim = c(0,100),
               break.x.by = 10,
...
full_plot
p26=full_plot
save(p26,file = './load/img/p26.Rdata')
...
tbl47 <- tbl_regression(res.cox, exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Multivariate Cox regression analysis for in hospital mortality of matched cohort5") 
tbl47
save(tbl47, file ="./load/tbl/tbl47.Rdata" )

## 森林图
p = ggforest(
  res.cox,
  data = matched_df,
  main = "Forest plot of multivariate Cox regression analysis for in hospital mortality of matched cohort5
...
p
p27=p
save(p27,file = './load/img/p27.Rdata')
