...
gp_select <- c("50% ≤ VRR", "0 ≤ VRR < 50%", "-50% ≤ VRR < 0", "VRR < -50%")[c(3,4)]
col_select <- c("#75D054FF", "#2C728EFF", "#FB5D0F", "#FB75D3")[c(3,4)]

f='./load/matched_cohort6.Rdata'
...
if (!file.exists(f)) {
  ...
  tbl_summary_matching = merge(tbl_before_matcing, tbl_after_matcing, by = 'Var1', all = T) %>% 
    ...
    flextable() %>% 
    set_caption(., "The number of SMD greater than 0.1 before and after matching for cohort6") 
  tbl_summary_matching
  tbl_smd6 = tbl_summary_matching
  save(tbl_smd6, file ="./load/tbl/tbl_smd6.Rdata" )
...
tbl
tbl48 <- tbl
save(tbl48, file ="./load/tbl/tbl48.Rdata")

# 匹配后table1
tbl = make_table1(fml,matched_df)
tbl
tbl49 <- tbl
save(tbl49, file ="./load/tbl/tbl49.Rdata")
...
tbl50 <- tbl_regression(res.svyglm,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted generalised linear model of with all covariates using IPW for ICU mortality of cohort6") 
tbl50
save(tbl50, file ="./load/tbl/tbl50.Rdata" )
...
tbl51 <- tbl_regression(res.svycox,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted Cox model with all covariates using IPW for ICU mortality of cohort6") 
tbl51
save(tbl51, file ="./load/tbl/tbl51.Rdata" )
...
## km曲线
p = ggsurvplot(fit,
               title = "Kaplan-Meier survival curve for ICU mortality of matched cohort6",
               xlab = "Time(Days)", 
...
full_plot
p28=full_plot
save(p28,file = './load/img/p28.Rdata')
...
tbl52 <- tbl_regression(res.cox, exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Multivariate Cox regression analysis for ICU mortality of matched cohort6") 
tbl52
save(tbl52, file ="./load/tbl/tbl52.Rdata" )

## 森林图
p = ggforest(
  res.cox,
  data = matched_df,
  main = "Forest plot of multivariate Cox regression analysis for ICU mortality of matched cohort6
...
p
p29=p
save(p29,file = './load/img/p29.Rdata')
...
tbl53 <- tbl_regression(res.svyglm,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted generalised linear model of with all covariates using IPW for in hospital mortality of cohort6") 
tbl53
save(tbl53, file ="./load/tbl/tbl53.Rdata" )
...
tbl54 <- tbl_regression(res.svycox,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted Cox model with all covariates using IPW for in hospital mortality of cohort6") 
tbl54
save(tbl54, file ="./load/tbl/tbl54.Rdata" )
...
## km曲线
p = ggsurvplot(fit,
               title = "Kaplan-Meier survival curve for in hospital mortality of matched cohort6",
               xlab = "Time(Days)", 
               ylab = "Surivival probability",
               xlim = c(0,100),
               break.x.by = 10,
...
full_plot
p30=full_plot
save(p30,file = './load/img/p30.Rdata')
...
tbl55 <- tbl_regression(res.cox, exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Multivariate Cox regression analysis for in hospital mortality of matched cohort6") 
tbl55
save(tbl55, file ="./load/tbl/tbl55.Rdata" )

## 森林图
p = ggforest(
  res.cox,
  data = matched_df,
  main = "Forest plot of multivariate Cox regression analysis for in hospital mortality of matched cohort6
...
p
p31=p
save(p31,file = './load/img/p31.Rdata')
