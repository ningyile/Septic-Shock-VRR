...
gp_select <- c("50% ≤ VRR", "0 ≤ VRR < 50%", "-50% ≤ VRR < 0", "VRR < -50%")[c(1,4)]
col_select <- c("#75D054FF", "#2C728EFF", "#FB5D0F", "#FB75D3")[c(1,4)]

f='./load/matched_cohort3.Rdata'
...
if (!file.exists(f)) {
  ...
  tbl_summary_matching = merge(tbl_before_matcing, tbl_after_matcing, by = 'Var1', all = T) %>% 
    ...
    flextable() %>% 
    set_caption(., "The number of SMD greater than 0.1 before and after matching for cohort3") 
  tbl_summary_matching
  tbl_smd3 = tbl_summary_matching
  save(tbl_smd3, file ="./load/tbl/tbl_smd3.Rdata" )
...
tbl
tbl24 <- tbl
save(tbl24, file ="./load/tbl/tbl24.Rdata")

# 匹配后table1
tbl = make_table1(fml,matched_df)
tbl
tbl25 <- tbl
save(tbl25, file ="./load/tbl/tbl25.Rdata")
...
tbl26 <- tbl_regression(res.svyglm,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted generalised linear model of with all covariates using IPW for ICU mortality of cohort3") 
tbl26
save(tbl26, file ="./load/tbl/tbl26.Rdata" )
...
tbl27 <- tbl_regression(res.svycox,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted Cox model with all covariates using IPW for ICU mortality of cohort3") 
tbl27
save(tbl27, file ="./load/tbl/tbl27.Rdata" )
...
## km曲线
p = ggsurvplot(fit,
               title = "Kaplan-Meier survival curve for ICU mortality of matched cohort3",
               xlab = "Time(Days)", 
...
full_plot
p16=full_plot
save(p16,file = './load/img/p16.Rdata')
...
tbl28 <- tbl_regression(res.cox, exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Multivariate Cox regression analysis for ICU mortality of matched cohort3") 
tbl28
save(tbl28, file ="./load/tbl/tbl28.Rdata" )

## 森林图
p = ggforest(
  res.cox,
  data = matched_df,
  main = "Forest plot of multivariate Cox regression analysis for ICU mortality of matched cohort3
...
p
p17=p
save(p17,file = './load/img/p17.Rdata')
...
tbl29 <- tbl_regression(res.svyglm,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted generalised linear model of with all covariates using IPW for in hospital mortality of cohort3") 
tbl29
save(tbl29, file ="./load/tbl/tbl29.Rdata" )
...
tbl30 <- tbl_regression(res.svycox,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted Cox model with all covariates using IPW for in hospital mortality of cohort3") 
tbl30
save(tbl30, file ="./load/tbl/tbl30.Rdata" )
...
## km曲线
p = ggsurvplot(fit,
               title = "Kaplan-Meier survival curve for in hospital mortality of matched cohort3",
               xlab = "Time(Days)", 
               ylab = "Surivival probability",
               xlim = c(0,100),
               break.x.by = 10,
...
full_plot
p18=full_plot
save(p18,file = './load/img/p18.Rdata')
...
tbl31 <- tbl_regression(res.cox, exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Multivariate Cox regression analysis for in hospital mortality of matched cohort3") 
tbl31
save(tbl31, file ="./load/tbl/tbl31.Rdata" )

## 森林图
p = ggforest(
  res.cox,
  data = matched_df,
  main = "Forest plot of multivariate Cox regression analysis for in hospital mortality of matched cohort3
...
p
p19=p
save(p19,file = './load/img/p19.Rdata')
