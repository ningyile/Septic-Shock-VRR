...
gp_select <- c("50% ≤ VRR", "0 ≤ VRR < 50%", "-50% ≤ VRR < 0", "VRR < -50%")[c(1,2)]
col_select <- c("#75D054FF", "#2C728EFF", "#FB5D0F", "#FB75D3")[c(1,2)]

f='./load/matched_cohort1.Rdata'
...
if (!file.exists(f)) {
  ...
  tbl_summary_matching = merge(tbl_before_matcing, tbl_after_matcing, by = 'Var1', all = T) %>% 
    ...
    flextable() %>% 
    set_caption(., "The number of SMD greater than 0.1 before and after matching for cohort1") 
  tbl_summary_matching
  tbl_smd1 = tbl_summary_matching
  save(tbl_smd1, file ="./load/tbl/tbl_smd1.Rdata" )
...
tbl
tbl8 <- tbl
save(tbl8, file ="./load/tbl/tbl8.Rdata")

# 匹配后table1
tbl = make_table1(fml,matched_df)
tbl
tbl9 <- tbl
save(tbl9, file ="./load/tbl/tbl9.Rdata")
...
tbl10 <- tbl_regression(res.svyglm,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted generalised linear model of with all covariates using IPW for ICU mortality of cohort1") 
tbl10
save(tbl10, file ="./load/tbl/tbl10.Rdata" )
...
tbl11 <- tbl_regression(res.svycox,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted Cox model with all covariates using IPW for ICU mortality of cohort1") 
tbl11
save(tbl11, file ="./load/tbl/tbl11.Rdata" )
...
## km曲线
p = ggsurvplot(fit,
               title = "Kaplan-Meier survival curve for ICU mortality of matched cohort1",
               xlab = "Time(Days)", 
...
full_plot
p8=full_plot
save(p8,file = './load/img/p8.Rdata')
...
tbl12 <- tbl_regression(res.cox, exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Multivariate Cox regression analysis for ICU mortality of matched cohort1") 
tbl12
save(tbl12, file ="./load/tbl/tbl12.Rdata" )

## 森林图
p = ggforest(
  res.cox,
  data = matched_df,
  main = "Forest plot of multivariate Cox regression analysis for ICU mortality of matched cohort1
...
p
p9=p
save(p9,file = './load/img/p9.Rdata')
...
tbl13 <- tbl_regression(res.svyglm,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted generalised linear model of with all covariates using IPW for in hospital mortality of cohort1") 
tbl13
save(tbl13, file ="./load/tbl/tbl13.Rdata" )
...
tbl14 <- tbl_regression(res.svycox,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted Cox model with all covariates using IPW for in hospital mortality of cohort1") 
tbl14
save(tbl14, file ="./load/tbl/tbl14.Rdata" )
...
## km曲线
p = ggsurvplot(fit,
               title = "Kaplan-Meier survival curve for in hospital mortality of matched cohort1",
               xlab = "Time(Days)", 
               ylab = "Surivival probability",
               xlim = c(0,100),
               break.x.by = 10,
...
full_plot
p10=full_plot
save(p10,file = './load/img/p10.Rdata')
...
tbl15 <- tbl_regression(res.cox, exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Multivariate Cox regression analysis for in hospital mortality of matched cohort1") 
tbl15
save(tbl15, file ="./load/tbl/tbl15.Rdata" )

## 森林图
p = ggforest(
  res.cox,
  data = matched_df,
  main = "Forest plot of multivariate Cox regression analysis for in hospital mortality of matched cohort1
...
p
p11=p
save(p11,file = './load/img/p11.Rdata')
