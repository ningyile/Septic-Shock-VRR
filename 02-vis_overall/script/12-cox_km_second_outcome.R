...
# VRR分组单变量
res.cox <- coxph(Surv(hos_los, hos_outcome) ~ Group, data = df)
res.cox
summary(res.cox)

tbl_regression(res.cox,exponentiate = TRUE) 

tbl5 <- tbl_regression(res.cox, exponentiate = TRUE) %>%
  as_flex_table() %>%
  set_caption(., "Univariate Cox regression for in hospital mortality") 

tbl5

save(tbl5, file ="./load/tbl/tbl5.Rdata" )
...
p = ggforest(
  res.cox,
  data = df,
  main = "Forest plot of univariate Cox regression analysis for in hospital mortality
...
p
p5=p
save(p5,file = './load/img/p5.Rdata')

# VRR分组生存曲线
fit <- survfit(Surv((hos_los), hos_outcome) ~ Group, data = df)

# km曲线
p = ggsurvplot(fit,
               title = "Kaplan-Meier survival curve for in hospital mortality of original cohort", 
               xlab = "Time(Days)", 
...
full_plot
p6=full_plot
save(p6,file = './load/img/p6.Rdata')


# 生存曲线两两比对
res_pairwise <- pairwise_survdiff(Surv(hos_los, hos_outcome) ~ Group, data = df)
res_pairwise

tbl6 = res_pairwise[["p.value"]] %>% 
  as.data.frame %>% 
  ...
  flextable() %>% 
  set_caption(., "Pairwise comparisons between groups for in hospital mortality survival curves") 

tbl6
save(tbl6, file ="./load/tbl/tbl6.Rdata" )
...
tbl7 <- tbl_regression(res.cox, exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Multivariate Cox regression analysis for in hospital mortality") 
tbl7
save(tbl7, file ="./load/tbl/tbl7.Rdata" )
...
p = ggforest(
  res.cox,
  data = df,
  main = "Forest plot of multivariate Cox regression analysis for in hospital mortality
...
p
p7=p
save(p7,file = './load/img/p7.Rdata')
