...
gp_select <- c("50% ≤ VRR", "0 ≤ VRR < 50%", "-50% ≤ VRR < 0", "VRR < -50%")[c(2,3)]
col_select <- c("#75D054FF", "#2C728EFF", "#FB5D0F", "#FB75D3")[c(2,3)]

f='./load/matched_cohort4.Rdata'
...
if (!file.exists(f)) {
  ...
  tbl_summary_matching = merge(tbl_before_matcing, tbl_after_matcing, by = 'Var1', all = T) %>% 
    ...
    flextable() %>% 
    set_caption(., "The number of SMD greater than 0.1 before and after matching for cohort4") 
  tbl_summary_matching
  tbl_smd4 = tbl_summary_matching
  save(tbl_smd4, file ="./load/tbl/tbl_smd4.Rdata" )
...
tbl
tbl32 <- tbl
save(tbl32, file ="./load/tbl/tbl32.Rdata")

# 匹配后table1
tbl = make_table1(fml,matched_df)
tbl
tbl33 <- tbl
save(tbl33, file ="./load/tbl/tbl33.Rdata")
...
tbl34 <- tbl_regression(res.svyglm,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted generalised linear model of with all covariates using IPW for ICU mortality of cohort4") 
tbl34
save(tbl34, file ="./load/tbl/tbl34.Rdata" )
...
tbl35 <- tbl_regression(res.svycox,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted Cox model with all covariates using IPW for ICU mortality of cohort4") 
tbl35
save(tbl35, file ="./load/tbl/tbl35.Rdata" )
...
## km曲线
p = ggsurvplot(fit,
               title = "Kaplan-Meier survival curve for ICU mortality of matched cohort4",
               xlab = "Time(Days)", 
...
full_plot
p20=full_plot
save(p20,file = './load/img/p20.Rdata')
...
tbl36 <- tbl_regression(res.cox, exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Multivariate Cox regression analysis for ICU mortality of matched cohort4") 
tbl36
save(tbl36, file ="./load/tbl/tbl36.Rdata" )

## 森林图
p = ggforest(
  res.cox,
  data = matched_df,
  main = "Forest plot of multivariate Cox regression analysis for ICU mortality of matched cohort4
...
p
p21=p
save(p21,file = './load/img/p21.Rdata')
...
tbl37 <- tbl_regression(res.svyglm,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted generalised linear model of with all covariates using IPW for in hospital mortality of cohort4") 
tbl37
save(tbl37, file ="./load/tbl/tbl37.Rdata" )
...
tbl38 <- tbl_regression(res.svycox,exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Survey-weighted Cox model with all covariates using IPW for in hospital mortality of cohort4") 
tbl38
save(tbl38, file ="./load/tbl/tbl38.Rdata" )
...
## km曲线
p = ggsurvplot(fit,
               title = "Kaplan-Meier survival curve for in hospital mortality of matched cohort4",
               xlab = "Time(Days)", 
               ylab = "Surivival probability",
               xlim = c(0,100),
               break.x.by = 10,
...
full_plot
p22=full_plot
save(p22,file = './load/img/p22.Rdata')
...
tbl39 <- tbl_regression(res.cox, exponentiate = TRUE) %>%
  as_flex_table() %>% 
  set_caption(., "Multivariate Cox regression analysis for in hospital mortality of matched cohort4") 
tbl39
save(tbl39, file ="./load/tbl/tbl39.Rdata" )

## 森林图
p = ggforest(
  res.cox,
  data = matched_df,
  main = "Forest plot of multivariate Cox regression analysis for in hospital mortality of matched cohort4
...
p
p23=p
save(p23,file = './load/img/p23.Rdata')
