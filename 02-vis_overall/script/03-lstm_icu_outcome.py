...
def data_to_sequence(x_train, y_train):
    ...
    encoded_labels = label_encoder.fit_transform(y_train.icu_outcome)
...
def train_run(x_train = [], y_train = [], x_test=[], y_test=[]):
    ...
    logger = TensorBoardLogger("lightning_logs", name="icu_outcome")
...
def write_predictions(cnt, labels, predictions, probs_of_pre, outcome_class_of_pre):
    f = open("./02-vis_overall/csv/vis_lstm_prediction/icu_outcome/prediction_%dh.csv" % cnt, "w", newline="")
    writer = csv.writer(f)
...
for x_index in range(1, 49):
    x_train = pd.read_csv("./02-vis_overall/csv/vis_lstm_matrix/icu_outcome/x_train_%dh.csv" % x_index)
    y_train = pd.read_csv("./02-vis_overall/csv/vis_lstm_matrix/icu_outcome/y_train_%dh.csv" % x_index)
    x_test = pd.read_csv("./02-vis_overall/csv/vis_lstm_matrix/icu_outcome/x_test_%dh.csv" % x_index)
    y_test = pd.read_csv("./02-vis_overall/csv/vis_lstm_matrix/icu_outcome/y_test_%dh.csv" % x_index)
...
f = open('./02-vis_overall/csv/vis_lstm_prediction/icu_outcome/accuracy.csv','w', newline="")
writer = csv.writer(f)
for row in result:
    writer.writerow(row)
f.close()
